...
class Admission_Details(BasePage):
    admissionIcon = (By.XPATH, "//div[@data-testid='ps-sidebar-container-test-id']/div/div[2]/nav/ul/li[7]/a")
    ...
    statusDD = (By.XPATH, "//div[@class='mb-4 relative w-full lg:max-w-[428px]']/button")
    completedOption = (By.XPATH, "//div[@class='mb-4 relative w-full lg:max-w-[428px]']/ul/li[1]")
    inReviewOption = (By.XPATH, "//div[@class='mb-4 relative w-full lg:max-w-[428px]']/ul/li[2]")
    shortlistedOption = (By.XPATH, "//div[@class='mb-4 relative w-full lg:max-w-[428px]']/ul/li[3]")
    rejectedOption = (By.XPATH, "//div[@class='mb-4 relative w-full lg:max-w-[428px]']/ul/li[4]")

    interviewStatus = (By.XPATH,
                       "//div[@class='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-[16px] ']/div[1]/div[3]/div/div[3]/div[1]/p")

    applyButton = (
        By.XPATH, "//div[@class='mt-[16px] self-end mb-[20px] flex flex-row-reverse w-[67px] ml-[116px]']/div/button")

    closeButton = (By.XPATH, "//div[@class='mb-4 relative w-full lg:max-w-[428px]']/button/div/div/div/div")
    ...
    submitButtonMul = (
        By.XPATH,
        "//button[@class='inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none bg-primary text-white hover:bg-primary/90 text-white disabled:text-ime-gray-200 disabled:!bg-ime-gray-400 disabled:opacity-100 h-10 px-6 w-full']")

    def __init__(self, driver,environment):
        self.environment = environment
        super().__int__(driver)

    def click_on_admission_icon(self):
        time.sleep(5)
        aIcon = self.driver.find_element(By.XPATH,
                                         "//div[@data-testid='ps-sidebar-container-test-id']/div/div[2]/nav/ul/li[11]/a")
        self.de_scroll_into_view(aIcon)
        time.sleep(1)
        self.de_click(self.admissionIcon)
        time.sleep(5)

    def fetch_overall_admission_interviews_details(self):
        log.logger.info("!!! == Assert that digit of Admission interviews displayed should match the total number of "
                        "interview"
                        "details list  == !!!")
    ...
    def fetch_interview_position_applicant_numbers_for_admission(self):
        log.logger.info(
            "!!! == Fetch the list of Admission interview position and applicant appeared for particular position == !!!")
    ...
    def traverse_admission_interview_list(self):
        log.logger.info(
            "!!! == Fetch the details of all interviewer appeared in the Admission interviews for same position  == !!!")
        totalNumberOfInterviews = self.get_element_text(self.numberOfInterview)
        number = totalNumberOfInterviews.split()
        numberOfInterviews = number[0]
        log.logger.info("Number Of Interviews == " + str(totalNumberOfInterviews))
    ...
    def filter_interview_details(self):
        log.logger.info(
            "!!! == Filter the details of all interviewer appeared in the Admission interview based on interview status  == !!!")
        totalNumberOfInterviews = self.get_element_text(self.numberOfInterview)
        number = totalNumberOfInterviews.split()
        numberOfInterviews = number[0]
        log.logger.info("Number Of Interviews == " + str(totalNumberOfInterviews))
    ...
    def filter_interview_details_through_search(self):
        log.logger.info(
            "!!! == Filter the details of all interviewer appeared in the Admission interview based on interviewer Name  == !!!")
        totalNumberOfInterviews = self.get_element_text(self.numberOfInterview)
        number = totalNumberOfInterviews.split()
        numberOfInterviews = number[0]
        log.logger.info("Number Of Interviews == " + str(totalNumberOfInterviews))
    ...
    def test_multimedia_resubmission(self):
        log.logger.info(
            "!!! == Resubmit  Multimedia Request Test Case For Audition == !!!")
        time.sleep(1)
        ...
        log.logger.info(
            "!!! == User Has clicked on view application button  == !!!")
        self.de_click(self.viewApplicationButtonForMultimedia)
        time.sleep(2)
        self.de_click(self.oneWayInterViewTab)
        log.logger.info(
    ...
    def test_update_interview_status(self):
        ...
        if validation == 'True':
            ...
            time.sleep(12)
            status = self.get_element_text(self.interviewStatus)
            log.logger.info("Status of interview Has Benn Updated to ===  " + status)
    ...
    def fetch_interview_transcript(self):
        log.logger.info("==== Verify and fetched transcription of Admission interviews ==== ")
        self.de_click(self.backApplicationPage)
        time.sleep(1.5)
        totalNumberOfInterviews = self.get_element_text(self.numberOfInterview)
        number = totalNumberOfInterviews.split()
    ...
    def test_create_multiway_of_admission(self):
        time.sleep(2)
        log.logger.info("!!! == Create Multiway Video Session From Admission Vertical  == !!!")
        totalNumberOfInterviews = self.get_element_text(self.numberOfInterview)
        number = totalNumberOfInterviews.split()
        numberOfInterviews = number[0]
        log.logger.info("Number Of Interviews == " + str(totalNumberOfInterviews))
            ...
            for i in range(totalList):
                ...
                if NameOfAction == "Multiway":
                    ...
                    if str(Status) == "True":
                        ...
                        try:
                            # Set up the WebDriverWait and attempt to click the element
                            ...
                            print("Element was found but could not be clicked.")
                        except Exception as e:
